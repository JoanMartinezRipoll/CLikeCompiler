...
infosym *symtab::obtainValueFromScope(string id, string scopeId){
  scope *sc=current;
  while (sc!=0) {
    if (sc->m.find(id)!=sc->m.end()) {
     if (sc->idtable == scopeId) return &sc->m[id];
    }
    sc=sc->previous;
  }
  return NULL;
...
void symtab::insertIdIntoScope(string id, string scopeId, string value){
  scope *sc=current;
  while (sc!=0) {
    if (sc->m.find(id)!=sc->m.end()) {
      if (sc->idtable == scopeId) { sc->m[id].value=value;}
    }
    sc=sc->previous;
  }
}
...
void symtab::write()
{
  ...
  while (sc!=0) {
    ...
    for (list<string>::iterator it=sc->ids.begin();
	 it!=sc->ids.end();it++) {
      cout<<*it<<":"<<sc->m[*it].kind<<","<<sc->m[*it].value;
