...
public:
  ...
  //modifica, en cas d'existir, un id d'un scope determinat
  void insertIdIntoScope(string id, string scopeId, string value);
  
  // Retorna el infosym de un id a un scope determinat de la symbol table o null si no l'ha trobat
  infosym *obtainValueFromScope(string id, string scopeId);
  
  // Retorna cert si troba un id a un scope determinat de la symbol table
  //bool idInScope(string id, string scopeId);
